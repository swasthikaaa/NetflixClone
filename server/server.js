...
// Periodic Persistent Notifications
...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        const user = new User({ email, password: hashedPassword, profile: { name: email.split('@')[0], avatar: '/images/avatar_red.webp' } });
        await user.save();
...
app.put('/api/user/profile', authenticateToken, async (req, res) => {
    try {
        ...
        await user.save();
        res.json({ message: 'Updated', user: { email: user.email, profile: user.profile, plan: user.plan } });
...
const processMovieItem = (movie) => {
    ...
    return {
        ...
        poster_path: p ? `https://image.tmdb.org/t/p/w500${p}` : '/images/stranger_things_poster.webp',
        backdrop_path: b ? `https://image.tmdb.org/t/p/original${b}` : '/images/stranger_things_poster.webp',
...
app.get('/api/movies/type/:category', async (req, res) => {
    ...
    if (!TMDB_API_KEY || TMDB_API_KEY === 'YOUR_TMDB_API_KEY_HERE') return res.json(fallback);
...
app.get('/api/movies/trending', async (req, res) => {
    if (!TMDB_API_KEY || TMDB_API_KEY === 'YOUR_TMDB_API_KEY_HERE') return res.json(MOCK_MOVIES);
    try {
